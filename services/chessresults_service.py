...
class Chessresults_Service:
    ...
    def __get_rank_from_chessreults_when_db_not_exists(self, key: str, round) -> Tuple[list, str]:
        try:
    ...
    def __get_rank_from_chessresult_when_db_exists(self, key: str, round) -> list:
        try:
            excel_url = get_chess_results_excel_link(key, round)
            rows = self.__get_chess_results_excel_rows(excel_url)
            return rows
    ...
    def __insert_tnr_to_db(self, key: str, round, rows: list) -> Tournament:
        try:
    ...
    def __get_tnr_result_from_key_and_round(self, key: str, round: int, db_tnr: Tournament = None):
        try:
    ...
    def get_tnr_result(self, key: str, round: int = None):
        try:
